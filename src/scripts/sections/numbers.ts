...
function renderNumbersSection(): void {
    ...
    numbersData.forEach(item => {
        ...
        const clickListener = () => {
            speakText(`${item.number} - ${item.word}`);
        ...
        eventListeners.push({ element: numberCard, type: 'click', listener: clickListener });
        ...
        eventListeners.push({ element: numberCard, type: 'keydown', listener: keydownListener });
